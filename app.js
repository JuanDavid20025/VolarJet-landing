...
/* FAQ: abrir de a una (opcional) */
$("#faq")?.addEventListener("toggle", (e)=>{
  if(e.target.tagName !== "DETAILS" || !e.target.open) return;
  $$("#faq details[open]").forEach(d => { if(d!==e.target) d.removeAttribute("open"); });
}, true);
...
function confettiBurst(count=100){
  ...
  for(let i=0;i<count;i++){
    const p = document.createElement("span"); p.className = "cft";
    const colors = ["#0ddb88","#7cf2c0","#22d3ee","#fbbf24","#f472b6"];
    p.style.background = colors[Math.floor(Math.random()*colors.length)];
    p.style.left = Math.random()*100 + "vw"; p.style.top  = "-10px"; p.style.opacity = 0.9;
    const size = 6 + Math.random()*6; p.style.width = size + "px"; p.style.height= size*0.6 + "px";
    p.style.transform = `rotate(${Math.random()*360}deg)`;
    p.style.animation = `fall ${2.2 + Math.random()*1.8}s linear forwards`; p.style.animationDelay = (Math.random()*0.2)+"s";
    fx.appendChild(p); setTimeout(()=> p.remove(), 4500);
  }
}
